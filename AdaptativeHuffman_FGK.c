...
Node* ajouteSymbole(unsigned char symbole, Node **zeroNoeud, Symbole **symboles) {
    ...
    Node *noeudDroit = ajoutefils(*zeroNoeud, 0, 0, symbole, 1, (*zeroNoeud)->ord - 1);
...
Node* trouveNoeud(Node *nEntree, Node *racine){
    Node *resultat = nEntree;
    if(racine->val > resultat->val && !racine->sFeuille){
        Node *gauche = trouveNoeud(resultat, racine->filsGauche);
        if (gauche) resultat = gauche;
        Node *droit = trouveNoeud(resultat, racine->filsDroit);
        if (droit) resultat = droit;
    }
    else if(racine->val == resultat->val && racine->ord > resultat->ord){
        resultat=racine;
    }
    return (resultat != nEntree) ? resultat : NULL;
}
...
unsigned char ajouteAuBuffer(int *code, int longueurDuCode, FILE *fp, unsigned char buffer, int *bufferSize) {
...
unsigned char ajouteOctet(char octet, FILE *fp, unsigned char buffer, int *bufferSize) {
    ...
    bufferActuel = bufferActuel | tempoctet;
    ...
    Symbole **symboles = calloc(symbolesPossibles, sizeof(Symbole*));
        ...
        while (fread(&octetActuel, sizeof(unsigned char), 1, fp_in) > 0) {
            ...
            if (symboleArbre) {
                ...
                buffer = ajouteOctet(octetActuel, fp_out, buffer, &bufferSize);
...
void decode(FILE *fp_in, FILE *fp_out) {
    ...
    while (!feof(fp_in)) {
        ...
        while (!noeudActuel->sFeuille && !finDuFichier) {
            int bit = lireBit(fp_in, &buffer, &bufferSize, fileSize, l);
            if (bit == 0) {
